    ...
    fun buildRequest_payloadShouldMatchWithGivenParameters() {
        val request = Authentication.AuthenticationRequestBuilder(
            ...
            areq = Authentication.AuthenticationRequestBuilder.AReq(
                ...
                sdkEphemPubKey = Authentication.AuthenticationRequestBuilder.SdkEphemPubKey(
                    kty = "EC",
                    x = "xxx",
                    y = "xxx",
                    crv = "P-256"
            ...
            // TODO: Device info from Netcetera's 3DS SDK
            // deviceInfo = "eyJhbGciOiJSU0EtT0FFUC0yNTYiLCJlbmMi",
            deviceInfo = Serializer().objectMapper.readValue("{}", Map::class.java) as Map<String, Any>,
        ).build()

        assertEquals("POST", request.method)
        assertEquals("https://www.omise.co/pay", request.url.toString())
        assertRequestBodyEquals(
            "{\"areq\":{\"sdkAppID\":\"co.omise.app\",\"sdkEphemPubKey\":{\"kty\":\"EC\",\"x\":\"xxx\",\"y\":\"xxx\",\"crv\":\"P-256\"},\"sdkTransID\":\"7f101033-df46-4f5c-9e96-9575c924e1e7\",\"sdkMaxTimeout\":5},\"device_info\":{},\"device_type\":\"Android\"}",
            request.payload!!
        )
    }
}
